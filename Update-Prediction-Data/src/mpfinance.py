...
def plot_day_summary_oclh(ax, quotes, ticksize=3,
    ...
    return _plot_day_summary(ax, quotes, ticksize=ticksize,
                             colorup=mcolors.to_rgb(colorup), colordown=mcolors.to_rgb(colordown),
                             ochl=True)
...
def _plot_day_summary(ax, quotes, ticksize=3,
    ...
    for q in quotes:
        ...
        if close >= open:
            color = mcolors.to_rgb(colorup)
        else:
            color = mcolors.to_rgb(colordown)
...
def _candlestick(ax, quotes, width=0.2, colorup='k', colordown='r',
                 alpha=1.0, ochl=True, linewidth=0.5):
    """
    Plot the time, open, high, low, close as a vertical line ranging
    from low to high.  Use a rectangular bar to represent the
    open-close span.  If close >= open, use colorup to color the bar,
    ...
    for q in quotes:
        ...
        rect = Rectangle(
            ...
            edgecolor=mcolors.to_rgb(color),
            alpha=None
        )
...
def candlestick2_ohlc(ax, opens, highs, lows, closes, width=4,
    ...
    rangeSegments_oc = [((i, open), (i, close))
                     for i, open, close in zip(xrange(len(opens)), opens, closes)
                     if open != -1 and close != -1]
    ...
    rangeCollection_oc = LineCollection(rangeSegments_oc,
                                     colors=colors,
                                     linewidths=width,
                                     antialiaseds=useAA,
                                     )
    ...
    # add these last
    ax.add_collection(rangeCollection)
    # ax.add_collection(rangeCollection_oc)
...
def index_bar(ax, vals,
    ...
    facecolors = (mcolors.to_rgb(facecolor),)
    edgecolors = (mcolors.to_rgb(edgecolor),)
